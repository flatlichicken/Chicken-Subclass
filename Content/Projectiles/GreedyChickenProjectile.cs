...
namespace Chickensubclass.Content.Projectiles
{
	...
	public class GreedyChickenProjectile : ModProjectile
	{
			...
			if (Projectile.penetrate <= 3) {
				if (Projectile.owner == Main.myPlayer) {
					Projectile.NewProjectile(
						...
						Projectile.Center, 
						Projectile.velocity, 
						ModContent.ProjectileType<GreedyChickenProjectile2>(),
						Projectile.damage, 
		...
		public override void PostAI() {
			...
			if (player.channel && !hasBeenReleased) {
				launchVelocity = Projectile.velocity;
			}
			else {
				hasBeenReleased = true;
				Projectile.velocity = Vector2.Normalize(launchVelocity) * 50f;
